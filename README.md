...
Prefer typing exact numbers? Bind "Edit Selection Coordinates" (Options > Controls > Area Scale, unbound by default) to open a minimal overlay showing just the six X/Y/Z values - no menu chrome, no dark background, the world and the wireframe box stay fully visible. Left-click a value to +1 it, right-click to -1 (hold shift for +/-10).
...
Place a Display Platform (3 stone slabs in a row) and right-click it with a filled capsule. The whole structure appears above it as a scaled diorama - genuinely any size, since it's rendered with display entities rather than real blocks. It rotates to face whichever way you were looking when you placed it, and an invisible barrier keeps mobs from wandering into or falling through it. Right-click empty handed to pick it back up.
...
- `config/areascale.json`: set `maxCaptureBlocks` (0 = unlimited) to cap how large a single capture can be, and optionally `commandAlias` to register a second command name server-wide.
- Each player can additionally set their own personal chat-prefill shorthand from an in-game settings screen (keybind, unbound by default).
...
- Captured entities in the diorama are reconstructed as real entities (no decorative "display" equivalent exists for entities), so they do have normal collision/AI unlike the block diorama around them.
